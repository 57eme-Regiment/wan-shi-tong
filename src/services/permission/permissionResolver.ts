import { z } from 'zod';
import { UserPermissionData } from './permissionResolver.schema';

/** Schéma et type représentant le résultat de la résolution des permissions d'un utilisateur. */
...
/**
 * Calcule les permissions effectives d'un utilisateur à partir de ses données brutes.
...
export function resolvePermissions(
  ...
  const permissions = [...new Set([...inherited, ...allowed])].filter(
    p => !denied.has(p),
  );
